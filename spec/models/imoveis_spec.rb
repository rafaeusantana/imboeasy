...
RSpec.describe Imovel, :type => :model do
  ...
  it "busca de imóveis" do
    imovel = Imovel.create!(cidade: "São Paulo", endereco: "Av. Paulista, 300", descricao: "Um lindo apartamento!", qtd_quartos: 3, metragem: 60, vagas_auto: 1, valor_aluguel: 2000, valor_condominio: 1200, tipo_imovel_id: 1, estado_id: 1, qtd_suites: 1, ano_construcao: 1980, bairro: "Jardim Paulista")

    imovels = Imovel.selecionarImoveis({cidade: "São Paulo", bairro: "Jardim Paulista"})

    expect(imovels[0]).to eq(imovel)
  end
end
